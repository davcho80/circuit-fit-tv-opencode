...
La session coach est conservée dans le navigateur via le JWT stocké en `localStorage`. La politique par défaut du backend signe les tokens pour 30 jours et peut être ajustée avec `JWT_EXPIRES_IN` ; l'interface affiche l'échéance dans le menu utilisateur et supprime automatiquement le token expiré. Les routes TV (`/tv`, `/pair`) restent publiques et n'envoient pas de session coach aux WebSocket TV.
...
- [Sécurité production](docs/security.md)
